...
WaveformDataChannel.prototype = {
  ...
  min_sample: function(index) {
    const offset = (index * this.waveformData.channels + this.channelIndex) * 2;

    return this.waveformData.adapter.at(offset);
  },
  ...
  max_sample: function(index) {
    const offset = (index * this.waveformData.channels + this.channelIndex) * 2 + 1;
   ...
   * var channel = waveform.channel(0);
   *
   * console.log(channel.min); // -> [-7, -5, -10]
   * ```
   *
   * @api
  ...
  get min() {
    return this.waveformData._offsetValues(
      this.waveformData.offset_start,
      this.waveformData.offset_length,
      this.channelIndex * 2
    );
  },
   ...
   * var channel = waveform.channel(0);
   *
   * console.log(channel.max); // -> [9, 6, 11]
   * ```
   *
   * @api
...
WaveformData.prototype = {
  ...
  get sample_rate() {
    return this.adapter.sample_rate;
  },
