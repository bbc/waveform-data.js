...
function WaveformData(options) {
  ...
  else if (isJsonWaveformData(options.data)) {
    var array_buffer = convertJsonToBinary(options.data);

    this._adapter = new WaveformDataArrayBufferAdapter(array_buffer);
  }
  else if (isBinaryWaveformData(options.data)) {
    this._adapter = new WaveformDataArrayBufferAdapter(options.data);
  }
  else {
    throw new TypeError(
